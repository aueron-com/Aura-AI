...
*Provide one or two well-explained approaches. Focus on clarity, correctness, and interview-readiness. Use {primary_language} for code examples unless the problem specifies otherwise.*

A.  **Approach 1: [RECOMMENDED/OPTIMAL SOLUTION]**
    1.  **Algorithm/Technique:** Briefly name or describe the algorithm/technique used (e.g., "Two Pointers", "BFS", "Dynamic Programming").
    2.  **Complexity Analysis:**
        -   Time Complexity: O(?)
        -   Space Complexity: O(?)
    3.  **Code Implementation ({primary_language}):**
        ```{(primary_language).lower()}
// Approach 1: [Brief description of this specific implementation]
// Time: O(?), Space: O(?)
...
// [Ensure the code is directly usable and well-formatted]
        ```
    4.  **Walkthrough:** Briefly explain how the code works with a small example.
    5.  **Why this approach?** Justify why this is a good solution (e.g., optimal, clear, common).

B.  **Approach 2: [ALTERNATIVE SOLUTION] (Optional, if relevant)**
    1.  **Algorithm/Technique:** Briefly name or describe.
    2.  **Complexity Analysis:**
        -   Time Complexity: O(?)
        -   Space Complexity: O(?)
    3.  **Code Implementation ({primary_language}):**
        ```{(primary_language).lower()}
// Approach 2: [Brief description of this specific implementation]  
// Time: O(?), Space: O(?)

// [Alternative implementation with comments for the coding problem]
        ```
    4.  **Walkthrough:** Brief explanation.
    5.  **Trade-offs:** Compare with Approach 1 (e.g., "Simpler but less efficient", "Better space complexity under certain conditions").
